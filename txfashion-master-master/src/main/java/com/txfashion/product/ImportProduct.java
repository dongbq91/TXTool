...
public class ImportProduct {
	...
	public static void generateProductImport(String[] designCodes, String imageFolder) throws IOException {
        ...
        for (String designCode : designCodes) {
        	...
        	String productSize = getProperty("product_size");
        	String des = "<ul> <li>Type: " + getProductType(getProperty("product_type")) + "</li> <li>Material: 80% Cotton / 20% Polyester</li> <li>Contrast colors sewn-in</li> <li>Embroidered fabric applique graphics</li> <li>Lacing at collar sewn-in place and non-adjustable</li> <li>The fabric is durable and resistant to wrinkles, shrinking and mildew.</li> <li>Midweight hoodie suitable for moderate temperatures</li> <li>Pullover</li> <li>Machine wash, features a specialty high definition heat-dye application that ensures long lasting color vibrancy even after machine washing. <li>Officially licensed</li> </ul>";
        	...
        	if (types.contains("Shoes")) {
        		des = "<ul> <li>Lightweight construction with breathable mesh fabric provides a comfortable and flawless fit.</li> <li>Lace-up closure for a snug fit.</li> <li>Features a removable memory foam insole pad for comfort and support.</li> <li>Finished with a high quality EVA out sole for traction and exceptional durability.</li> </ul>";
        	}
        	
        	if (generateVariant(productImages, designCode, productId, post_name, variantPrinter)) {
        		// Print Product
            	csvPrinter.printRecord(productName, post_name.trim(), productId, "", des, "publish", 0, "03/03/2019  12:00:00 AM", 0, 1, "open", "", "no", "no", "visible", "", "instock", "no", "no", "", "", "", "", "", "", "taxable", "", "", "", "no", "", "", 0, 0, "", "", "", "", "", "", productImage, "", "variable", "exclude_from_catalog|exclude_from_search", categoryName, tagName, "", "", "", "", "", "", "", 0, types, "0|1|1", "", productSize.replace(",", " | "), "1|1|1", "");
	...
	public static boolean generateVariant(Map<String, String> productImages, String designCode, long productId, String post_name, CSVPrinter variantPrinter) throws IOException {
		...
		for (String typeContent : typesContents) {
			...
			for (String size : sizes) {
				...
				// Print Product
				String sku = post_name + "-" + code + "-" + size;
				
				// END FO SHOES
				
				sku = sku.replaceAll("\\s","").trim();
				variantPrinter.printRecord(getProperty("product_name"), "", productId, variantId, "publish", menuOrder, sku, "no", "no", "", "instock", price, salePrice, "", "", "", "", "parent", "", "", "{\"d41d8cd98f00b204e9800998ecf8427e\":{\"name\":\"\",\"file\":\"\"}}", 0, variantImage, "::", "", "", "", "", "", type, size, "", "", "", "", "", 0); 
			}
		}
